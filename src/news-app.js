...
import { scroll as LayoutScroll } from "../node_modules/@polymer/app-layout/helpers/helpers.js";
...
class NewsApp extends PolymerElement {
  static get template() {
    ...
    <news-analytics key="UA-"></news-analytics>
    ...
    -->
    <news-data id="data" route=[[route]] categories="{{categories}}" category="{{category}}" article="{{article}}" loading="{{loading}}" offline="[[offline]]" failure="{{failure}}"></news-data>

    <news-nav id="nav" app-title="[[appTitle]]" page="[[page]]" categories="[[categories]]" category="[[category]]" load-complete="[[loadComplete]]">
      [[articleHeadline]]
    </news-nav>

    <iron-pages id="pages" role="main" attr-for-selected="name" fallback-selection="path-warning">
      <!-- list view of articles in a category -->
      <news-list name="categoria" category="[[category]]" loading="[[loading]]" offline="[[offline]]" failure="[[failure]]"></news-list>
      <!-- article view -->
      <news-article name="articulo" category="[[category]]" article="[[article]]" loading="[[loading]]" offline="[[offline]]" failure="[[failure]]"></news-article>
...
`;
  ...
  ready() {
    ...
    this.removeAttribute('unresolved'); // Chrome on iOS recomputes "vh" unit when URL bar goes off screen. Since we use "vh" unit
  ...
  _routePageChanged(page, isAttached) {
    ...
    if (!page) {
      ...
      this.set('route.path', 'categoria/top');
      return;
    }

    this.page = page; // Scroll to the top of the page on every *route* change. Use `Polymer.AppLayout.scroll`
  ...
  _pageChanged(page, oldPage) {
    ...
    switch (page) {
      case 'categoria':
        import('./news-list.js').then(cb, cb);
        this.$.pages.select('categoria');
        break;

      default:
        import('./news-article.js').then(cb, cb);
        this.$.pages.select('articulo');
        break;
    }
  }
    ...
    if (!this.loadComplete) {
      afterNextRender(this, () => {
        import('./lazy-resources.js').then(() => {
          ...
          this.loadComplete = true; // Load pre-caching Service Worker
  ...
  _notifyNetworkStatus() {
    ...
    this._setOffline(!window.navigator.onLine); // Show the snackbar if the user is offline when starting a new session
    ...
    if (this.offline || !this.offline && oldOffline === true) {
      ...
      this._networkSnackbar.textContent = this.offline ? 'You are offline' : 'You are online';
  ...
  _updateDocumentTitle(page, categoryTitle, articleHeadline, appTitle) {
    document.title = (page === 'categoria' ? categoryTitle : articleHeadline) + ' - ' + appTitle;
  ...
  _setPageMetadata(description, article) {
    let image = article ? article.imageUrl : 'images/news-icon-128.png'; // Set open graph metadata
    ...
    this._setMeta('property', 'og:image', this.baseURI + image); // Set twitter card metadata
